...
| **C2. Distinguish "broken" vs "blocked-by-target"** | ◐ Partial (2026-07-20, `phase2-h-js-sites`): **429 / 503+Retry-After** now handled as rate-limiting, not broken — `httpChecker.js` honours `Retry-After`, applies escalating per-host cooldown + a circuit breaker (module-level `hostThrottle`), and emits `rateLimited` results kept OUT of `broken_links`; the job is flagged `rateLimitedDetected` → results-page banner. Remaining: **403 / Cloudflare JS-challenge** detection (needs body inspection — a `403` interstitial still counts as broken). | S |
