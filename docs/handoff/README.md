...
| 2026-07-10 | `phase2-ui-restructure` | **Brand icons wired + `.gitignore` bug fixed; landing wrapped.** Root cause of "icons not in git": the template `.gitignore` had a bare `public` line (Gatsby build-dir rule) — the whole Next.js `public/` folder was untracked. Fixed; `public/design/` (1 MB ChatGPT design refs) stays ignored — **note: anything in `public/` is publicly served; move design refs out before deploy.** User's angular-S logomark PNGs committed; header/footer now use `/seo-scrub-app-logo.png` via `next/image` (chain-link placeholder SVG chip deleted); favicon = `src/app/icon.png` (old default `favicon.ico` + unused starter SVGs deleted). Also diagnosed "gel effect not visible": `.btn-gel` WAS wired correctly but Turbopack dev served a **stale CSS chunk** (same URL, old content — HTML had the class, stylesheet lacked the rule); touching `globals.css` forced a recompile and it renders. If a CSS edit seems to not apply in dev, force-recompile before debugging. **Queued: motion elements** (user: "we will do that later"). Landing page is now considered wrapped; next surface = `/analyze` (step 3). |
| 2026-07-10 | `phase2-ui-restructure` | **Setup-page feedback pass — two design rules refined live (user).** (1) **Inputs sit on `bg-surface`, never `bg-bg`** — a `bg-bg` field disappears into the page / reads dingy inside a white card; mint/green input fills rejected (reads as validation state; green stays ink). Applied to `/analyze` URL input + depth select and `LargeCrawlForm` URL input + select. (2) **Containers are SQUARE** — user flagged rounded cards; audit of the token scale showed `rounded-lg` = **10px**, already over the documented 4–6px. Rule now codified (README non-negotiables + memory): cards/panels/option tiles/notice boxes/modals = radius 0 hairline boxes; 4–6px only on interactive controls; gel primaries keep `rounded-lg`. Squared across `/analyze`, landing (report excerpt, mode cards, Quick Check panel), `LargeCrawlForm` (tiles, notices, stop modal), `SecurityNotice`. Screenshot-verified. |
| 2026-07-10 | `phase2-ui-restructure` | **Step 3 landed — `/analyze` rebuilt as the Audit Setup page (doc 06 §7).** The 3-step wizard (analyze → crawl → results in one page) is gone: `/analyze` is now setup-only; **Start Audit navigates to `/results/[jobId]`**, which already polls status while running (progress polish = step 4). Two-column layout: main = numbered field groups (01 URL + optional **"Estimate scope"** secondary button, 02 audit type radio cards Full Audit/Quick Check, 03 advanced disclosure w/ depth-band leader readout) + `.btn-gel` Start Audit; supporting = **Scope Estimate** dotted-leader readout (dashes until sampled; stale-URL + JS-heavy-site warnings) + "Your Report Includes." The pre-scan (old URL Structure Analyzer) is demoted to the estimate step — its `preAnalyzedUrls` still ride a Full Audit start (`usePreAnalyzedUrls` + `crawlMode:'content_pages'`, backend contract unchanged). **New: "Previous Audits" ledger** (user-approved) — `db.getRecentJobsWithCounts()` (raw SQL w/ broken/total counts) + `GET /api/jobs/recent` (`force-dynamic`); rows = mono hostname · leader · date · status · broken count · View report link. **Deleted: `UrlAnalyzer.js` (1,648 lines) + orphaned `CrawlForm.js`.** ESLint clean; screenshot-verified light+dark (headless Chrome; note: URL-embedded Basic Auth doesn't reach client `fetch`, so the ledger shows its error state in headless shots only — API verified 200 separately). Next surface = step 4, `/results/[jobId]` (confirm doc 09 report scope first). |
...
3. ~~**Audit setup `/analyze`**~~ ✅ landed 2026-07-10 — setup-only page (wizard removed; Start Audit
   → `/results/[jobId]`), scope-estimate sidebar, Previous Audits ledger (`/api/jobs/recent`).
   `UrlAnalyzer` and `CrawlForm` deleted.
...
- **Visual direction is "Typeset Audit"** (user-chosen 2026-07-09, after rejecting the generic
  "white cards + green accent" SaaS look as AI-template): the site looks like the typeset audit
  report it produces. Instrument Serif for display headlines (`font-display`), Inter for UI, JetBrains
  Mono for URLs/numerals/micro-labels (`font-mono`); flat surfaces with 1px hairline rules instead of
  shadows; **containers (cards/panels/notes/modals) are square — no `rounded-*`** (refined 2026-07-10;
  4–6px radii only on interactive controls: inputs/selects/secondary buttons; gel primaries keep
  `rounded-lg`); inputs sit on `bg-surface`, never `bg-bg`; numbered sections (01/02/03);
  left-aligned asymmetric layouts; green used as
  auditor's ink (underlines, rules, marks — not big fills). Doc 07's visual values are superseded where
  they conflict; its component inventory and doc 06 IA still apply. Kill-list of AI-template tells:
  centered hero, three icon-chip cards, gradient text, rounded-2xl everything, purple/indigo.
